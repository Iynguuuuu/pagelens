...
**Live Demo:** https://pagelens-37fb.vercel.app  
**Backend API:** https://pagelens-9118.onrender.com
...
2. **AI Insights** — Llama 3.3 70B (via Groq) analyzes the metrics + page text and returns structured JSON covering SEO, messaging, CTAs, content depth, and UX
...
[Flask Backend]
  ...
  ├── ai_analyzer.py    → Builds prompts, calls Groq API, parses JSON output
  ├── logger.py         → Saves full prompt log (system prompt, user prompt, raw output)
  └── app.py            → Orchestrates scrape → analyze → respond
      │
      ▼
[Groq API]  (llama-3.3-70b-versatile, structured JSON output)
```
...
Rather than asking the model to produce freeform text, the system prompt specifies an exact JSON schema. The model is instructed to return **only** valid JSON — no markdown fences, no preamble. This avoids fragile regex parsing and makes the output directly usable by the frontend.

### Metrics-Grounded Insights
The user prompt embeds the full metrics JSON alongside the page text. The system prompt explicitly requires every insight to **reference specific numbers**. This prevents generic AI output like "your content could be improved" — instead the model says "with only 1 H1 and 312 words, the page lacks the depth expected for competitive SERPs."

### Token Efficiency
Page text is capped at ~3,000 words before being sent to the model. This keeps input tokens predictable and cost-efficient while giving the model enough content to assess messaging clarity and content depth meaningfully.
...
| Structured JSON output | Reliable schema, but occasionally the model adds markdown fences — handled with a strip step. |
...
| Flask + Render free tier | Simple deployment, but cold starts can add ~30s to the first request after inactivity. |
...
3. Add environment variable: `VITE_API_URL=https://pagelens-9118.onrender.com`
4. Deploy
...
- `raw_model_output` — the model's response before JSON parsing
